...
function Remove-OldGuestUsers {
    ...
    process {
        ...
        $Return = foreach ($Guest in $CleanupGuests) {
            $DeleteCounter++
            Write-Progress -Activity "Deleting User" -Status $Guest.UserPrincipalName -PercentComplete (($DeleteCounter / $CleanupGuests.Count) * 100)
            
            if ($PSCmdlet.ShouldProcess($Guest.DisplayName,'Remove-MgBetaUser')) {
